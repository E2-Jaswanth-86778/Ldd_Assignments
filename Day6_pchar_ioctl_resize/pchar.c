...
case FIFO_RESIZE:
{
    ...
    // Allocate a temporary buffer to store FIFO contents
    temp_len = kfifo_len(&mybuf); // Get current FIFO length
    temp_buf = kmalloc(temp_len, GFP_KERNEL);
    if (!temp_buf) {
        printk(KERN_ERR "%s: kmalloc() failed in FIFO_RESIZE.\n", THIS_MODULE->name);
        return -ENOMEM;
    }

    // Copy the FIFO contents to the temporary buffer
    if (kfifo_out(&mybuf, temp_buf, temp_len) != temp_len) {
        printk(KERN_ERR "%s: kfifo_out() failed to retrieve all data in FIFO_RESIZE.\n", THIS_MODULE->name);
        kfree(temp_buf);
        return -EFAULT;
    }

    // Free the current FIFO memory
    kfifo_free(&mybuf);

    // Allocate new FIFO memory of the desired size
    ret = kfifo_alloc(&mybuf, param, GFP_KERNEL);
    if (ret != 0) {
        printk(KERN_ERR "%s: kfifo_alloc() failed in FIFO_RESIZE.\n", THIS_MODULE->name);
        kfree(temp_buf); // Free the temporary buffer on failure
        return ret;
    }

    // Copy contents from the temporary buffer back to the FIFO
    kfifo_in(&mybuf, temp_buf, temp_len);
